...
export default function ValidationH({ validationResponse = null }) {
  ...
  const renderTaskCompletion = () => {
    if (!taskCompletion || Object.keys(taskCompletion).length === 0)
      return null;
    return (
      <div className="mb-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {Object.entries(taskCompletion).map(([key, value]) => (
            <div
              key={key}
              className="bg-[var(--validation-box-bg)] rounded-lg p-4 border border-white/10"
            >
    ...
    <>
      ...
      <div className="bg-[var(--validation-box-bg)] backdrop-blur-sm rounded-xl border-2 border-purple-500/30 shadow-xl p-6 w-full max-w-6xl mx-auto">
        ...
        <div className="bg-[var(--validation-box-bg)] rounded-lg p-4 border border-white/10">
          ...
          <p className="text-[var(--text-muted)]">{overallRecommendation}</p>
        </div>
      </div>
    </>
  );
