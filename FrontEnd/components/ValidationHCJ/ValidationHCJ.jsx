...
export default function ValidationHCJ({ validationResponse }) {
    ...
    const getScoreColor = (score, outOf) => {
        if (score === "N/A") return "text-[var(--text-muted)]";
        if (score === 0) return "text-red-500";
        if (score < (outOf / 2)) return "text-yellow-500";
        ...
        return Object.entries(code).map(([section, sectionObj]) => (
            <div key={section} className="mb-6">
                ...
                {Object.entries(sectionObj).map(([key, value]) => (
                    <div key={key} className="mb-2 ml-4 bg-[var(--validation-box-bg)] rounded-lg p-4 border border-white/10">
                    ...
                    {Object.entries(taskCompletion).map(([key, value]) => (
                        <div key={key} className="bg-[var(--validation-box-bg)] rounded-lg p-4 border border-white/10">
                            ...
                            <div className="flex items-center gap-2 mb-2">
                                <div className={`text-xl font-bold ${getScoreColor(value.score, value.outOf)}`}>
